var v = {
	...
	removeMapItems:function(){
		...
		for (var polygon in v.polygons){
			v.map.removeLayer(v.polygons[polygon]);
		}
	...
	sectorInfo:function(mno,enb,sectors){
		...
		if (mno === 10){
			...
			if (v.findItem(sectors,[15,16,17])) ret += "?? ";
	...
	placePoints:function(data){
		for (var i = 0;i<data.length;i++){
			...
			for (var s in data[i].sectors){
				...
				v.polygons.push(
					L.polyline([
						[data[i].sectors[s][0],data[i].sectors[s][1]],
						[tLat,tLng]
					],{color: 'red'})
				);
			}
			...
			v.markers.push(
				...
				).bindTooltip(v.sectorInfo(v.mno,data[i].id,Object.keys(data[i].sectors)),
				{
					permanent: true,
					direction: 'bottom'
				})
		...
		for (var polygon in v.polygons){
			v.map.addLayer(v.polygons[polygon]);
		}
	}
};
