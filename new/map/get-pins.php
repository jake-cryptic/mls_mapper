...
$get_enodebs = $db_connection->query("SELECT enodeb_id,lat,lng FROM masts WHERE " . $conditions . " LIMIT 1000");

$get_sectors = $db_connection->prepare("SELECT sector_id,created,updated FROM sectors2 WHERE mnc = {$_GET['mno']} AND enodeb_id = ? LIMIT 50");
...
while ($node = $get_enodebs->fetch_object()){
	...
	$get_sectors->bind_result($resSectorId,$resTimeCreated,$resTimeUpdated);
	
	$sectorList = array();
	while($get_sectors->fetch()){
		$sectorList[$resSectorId] = array($resTimeCreated,$resTimeUpdated);
	}
